...
ENV SPEC_OPENBLAS="cp2k@master%gcc +sirius +elpa +libxc +libint smm=libxsmm +spglib +cosma +pexsi +plumed +libvori +openmp ^openblas+fortran ^dbcsr@2.6.0~shared+mpi+openmp ^cosma+shared~tests~apps"
ENV SPEC_MKL="cp2k@master%gcc +sirius +elpa +libxc +libint smm=libxsmm +spglib +cosma +pexsi +plumed +libvori +mpi +openmp ^intel-oneapi-mkl+cluster ^dbcsr@2.6.0~shared+mpi+openmp ^cosma+shared~tests~apps"
