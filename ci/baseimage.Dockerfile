...
ENV SPEC_OPENBLAS="cp2k@develop%gcc build_type=Release build_system=cmake +fftw +sirius +elpa +libxc +libint +smm=libxsmm +spglib +cosma +pexsi +plumed +libvori +openmp ^openblas+fortran ^dbcsr@develop"
ENV SPEC_MKL="cp2k@develop%gcc build_type=Release build_system=cmake +fftw +sirius +elpa +libxc +libint +smm=libxsmm +spglib +cosma +pexsi +plumed +libvori +mpi +openmp ^intel-oneapi-mkl+cluster ^dbcsr@develop"
